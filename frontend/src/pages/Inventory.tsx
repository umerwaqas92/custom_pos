                ...
                paginatedProducts.map((p) => {
                  ...
                  return (
                    <tr key={p.id} className={`hover:bg-secondary/20 transition ${selectedIds.has(p.id) ? "bg-primary/5" : ""}`}>
                      ...
                      <td className="py-4 text-right text-muted-foreground">Rs. {Number(p.purchasePrice).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      <td className="py-4 text-right font-semibold text-foreground">Rs. {Number(p.sellingPrice).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
