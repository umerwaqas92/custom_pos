...
// Create a Repair Job
router.post("/", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER", "CASHIER", "TECHNICIAN"), async (req, res) => {
    ...
    try {
        const job = await db_1.default.repairJob.create({
            data: {
                deviceName,
                imei: imei || null,
...
router.get("/", auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        const jobs = await db_1.default.repairJob.findMany({
            where,
            include: {
                customer: true,
                technician: true
...
router.get("/:id", auth_1.protect, async (req, res) => {
    const { id } = req.params;
    try {
        const job = await db_1.default.repairJob.findUnique({
            where: { id },
            include: {
                customer: true,
                technician: true
...
router.put("/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER", "TECHNICIAN"), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        if (partsUsed)
            data.partsUsed = JSON.stringify(partsUsed); // expected array
        if (repairCost !== undefined)
            data.repairCost = Number(repairCost);
        if (serviceCharge !== undefined)
            data.serviceCharge = Number(serviceCharge);
        if (estimatedDelivery)
            data.estimatedDelivery = new Date(estimatedDelivery);
        if (notes !== undefined)
            data.notes = notes;
        const updated = await db_1.default.repairJob.update({
            where: { id },
            data,
...
router.post("/warranty-claims", auth_1.protect, async (req, res) => {
    const { saleId, productId, notes } = req.body;
    if (!saleId || !productId) {
        return res.status(400).json({ error: "Sale ID and Product ID are required." });
    }
    try {
        const claim = await db_1.default.warrantyClaim.create({
            data: {
                saleId,
                productId,
                status: "PENDING",
...
router.get("/warranty-claims", auth_1.protect, async (req, res) => {
    try {
...
router.put("/warranty-claims/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER", "TECHNICIAN"), async (req, res) => {
    const { id } = req.params;
    const { status, resolutionDetails, notes } = req.body;
    try {
        const updated = await db_1.default.warrantyClaim.update({
            where: { id },
            data: {
                status,
                resolutionDetails,
