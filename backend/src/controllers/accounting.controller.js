...
// ==================== CUSTOMER MANAGEMENT ====================
// List Customers
router.get("/customers", auth_1.protect, async (req, res) => {
    try {
        const list = await db_1.default.customer.findMany({ orderBy: { name: "asc" } });
 ...
 */
router.get("/customers/:id/statement", auth_1.protect, async (req, res) => {
    const { id } = req.params;
    try {
        ...
        const [sales, creditPayments] = await Promise.all([
            ...
            db_1.default.customerCreditPayment.findMany({
                where: { customerId: id },
        ...
        const outstandingOnInvoices = sales.reduce((s, x) => s + Math.max(0, (x.payableAmount || 0) - (x.paidAmount || 0)), 0);
        return res.json({
            customer,
            sales,
            creditPayments,
...
// Create Customer
router.post("/customers", auth_1.protect, async (req, res) => {
    const { name, phone, email, address, creditLimit, notes } = req.body;
    if (!name || !phone)
        return res.status(400).json({ error: "Name and phone number are required." });
    try {
        const exists = await db_1.default.customer.findUnique({ where: { phone } });
        if (exists)
...
// Update Customer
router.put("/customers/:id", auth_1.protect, async (req, res) => {
    const { id } = req.params;
    const { name, phone, email, address, creditLimit, notes } = req.body;
    try {
...
router.post("/customers/:id/repay", auth_1.protect, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const paymentResult = await db_1.default.$transaction(async (tx) => {
...
// ==================== SUPPLIER MANAGEMENT ====================
// List Suppliers
router.get("/suppliers", auth_1.protect, async (req, res) => {
    try {
        const suppliers = await db_1.default.supplier.findMany({ orderBy: { company: "asc" } });
...
// Create Supplier
router.post("/suppliers", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
...
// Update Supplier
router.put("/suppliers/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    const { id } = req.params;
    const { company, contactPerson, phone, email, address } = req.body;
    try {
        const updated = await db_1.default.supplier.update({
            where: { id },
            data: { company, contactPerson, phone, email, address }
        });
        return res.json(updated);
...
// Delete Customer
router.delete("/customers/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER"), async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.default.customerCreditPayment.deleteMany({ where: { customerId: id } });
...
router.post("/customers/bulk-delete", auth_1.protect, (0, auth_1.restrictTo)("OWNER"), async (req, res) => {
    const { ids } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
        return res.status(400).json({ error: "No customer IDs provided." });
    }
    try {
        await db_1.default.customerCreditPayment.deleteMany({ where: { customerId: { in: ids } } });
        await db_1.default.customer.deleteMany({ where: { id: { in: ids } } });
        (0, cache_1.invalidateCache)("reports:");
...
// Delete Supplier
router.delete("/suppliers/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER"), async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.default.supplierPayment.deleteMany({ where: { supplierId: id } });
...
router.post("/suppliers/bulk-delete", auth_1.protect, (0, auth_1.restrictTo)("OWNER"), async (req, res) => {
    const { ids } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
        return res.status(400).json({ error: "No supplier IDs provided." });
    }
    try {
        await db_1.default.supplierPayment.deleteMany({ where: { supplierId: { in: ids } } });
        await db_1.default.supplier.deleteMany({ where: { id: { in: ids } } });
...
router.get("/purchases", auth_1.protect, async (req, res) => {
    try {
        const list = await db_1.default.purchaseOrder.findMany({
            include: {
                supplier: true,
...
router.post("/purchases", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER", "WAREHOUSE"), async (req, res) => {
    ...
    try {
        ...
        const order = await db_1.default.purchaseOrder.create({
            data: {
                supplierId,
                totalAmount: total,
                status: "PENDING",
...
router.put("/purchases/:id/status", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER", "WAREHOUSE"), async (req, res) => {
    const { id } = req.params;
    const { status, branchId } = req.body; // branchId is where stocks should be routed
    if (!status)
        return res.status(400).json({ error: "Status is required." });
    try {
        const purchase = await db_1.default.purchaseOrder.findUnique({
            where: { id },
            include: { items: true }
        });
        if (!purchase)
        ...
        const updatedOrder = await db_1.default.$transaction(async (tx) => {
            ...
            if (status === "RECEIVED") {
                if (!branchId)
                    throw new Error("A destination branch must be provided to receive items into stock.");
...
// ==================== EXPENSE TRACKING ====================
// List Expenses
router.get("/expenses", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    try {
        const expenses = await db_1.default.expense.findMany({ orderBy: { date: "desc" } });
...
// Add Expense Record
router.post("/expenses", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
...
// ==================== BANK ACCOUNTS ====================
// List Bank Accounts
router.get("/bank-accounts", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    try {
        const accounts = await db_1.default.bankAccount.findMany({
...
// Create Bank Account
router.post("/bank-accounts", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    const { name, type, accountNumber, bankName, notes } = req.body;
    if (!name || !type)
        return res.status(400).json({ error: "Name and type are required." });
    try {
        const account = await db_1.default.bankAccount.create({
...
// Update Bank Account
router.put("/bank-accounts/:id", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    const { id } = req.params;
    const { name, type, accountNumber, bankName, notes, isActive } = req.body;
    try {
        const updated = await db_1.default.bankAccount.update({
            where: { id },
...
router.get("/transactions", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    const { bankAccountId, type, category, startDate, endDate } = req.query;
    try {
        const where = {};
        if (bankAccountId)
            where.bankAccountId = String(bankAccountId);
        if (type)
        ...
        if (startDate || endDate) {
            ...
            if (endDate)
                where.createdAt.lte = new Date(String(endDate) + "T23:59:59.999Z");
        }
...
router.post("/transactions", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
...
router.post("/transactions/transfer", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
...
// ==================== PROFIT & LOSS REPORT ====================
router.get("/profit-loss", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    ...
    try {
        ...
        // Sales Revenue
        const salesAgg = await db_1.default.sale.aggregate({
            where: {
        ...
        const soldItems = await db_1.default.saleItem.findMany({
...
// ==================== DAILY CLOSING ====================
// List Daily Closings
router.get("/daily-closings", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    try {
        const closings = await db_1.default.dailyClosing.findMany({
...
router.post("/daily-closings", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    ...
    try {
        ...
        const existing = await db_1.default.dailyClosing.findFirst({
            where: { closingDate: { gte: startOfDay, lte: endOfDay } }
        });
        if (existing)
            return res.status(400).json({ error: "Daily closing already exists for this date." });
        ...
        const expenseAgg = await db_1.default.expense.aggregate({
            where: { date: { gte: startOfDay, lte: endOfDay } },
            _sum: { amount: true }
        });
        // Fetch returns/refunds for the day
        const returnsAgg = await db_1.default.saleReturn.aggregate({
            where: {
                status: "COMPLETED",
        ...
        const closing = await db_1.default.dailyClosing.create({
            data: {
...
router.get("/daily-closings/preview", auth_1.protect, (0, auth_1.restrictTo)("OWNER", "MANAGER"), async (req, res) => {
    ...
    try {
        ...
        const prevClosing = await db_1.default.dailyClosing.findFirst({
