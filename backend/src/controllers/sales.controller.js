...
router.post("/", auth_1.protect, async (req, res) => {
    const { customerId, items, // array of { productId, quantity, discount, tax, serialNumber, imei }
    paymentMethod, // 'CASH', 'CARD', 'MOBILE', 'SPLIT', 'CREDIT'
    paidAmount, discountAmount, // overall cart discount
    ...
    if (branchId) {
        const branchExists = await db_1.default.branch.findUnique({
            where: { id: branchId },
            select: { id: true }
        });
        if (!branchExists) {
    ...
    const activeBranch = await db_1.default.branch.findUnique({
        where: { id: branchId },
        select: { id: true }
    });
    if (!activeBranch) {
    ...
    const taxSettings = await db_1.default.systemSetting.findMany({
    ...
    try {
        ...
        const saleResult = await db_1.default.$transaction(async (tx) => {
            ...
            if (paymentMethod === "CREDIT") {
                if (!customerId)
                    throw new Error("Customer profile is required for credit transactions.");
                debt = payableAmount;
                paymentStatus = "UNPAID";
            }
            else if (paymentMethod === "EMI") {
                if (!customerId)
                    throw new Error("Customer profile is required for EMI transactions.");
...
const refundMethodToAccountType = {
    CASH: "CASH",
    CARD: "BANK",
    MOBILE: "MOBILE_WALLET"
};
// List all completed returns (must be before /:id)
router.get("/returns", auth_1.protect, async (req, res) => {
    ...
    try {
        ...
        if (branchId)
            where.sale = { branchId: String(branchId) };
        const returns = await db_1.default.saleReturn.findMany({
            where,
            include: {
...
router.get("/returns/:returnId", auth_1.protect, async (req, res) => {
    try {
        const saleReturn = await db_1.default.saleReturn.findUnique({
...
router.get("/:id/returnable", auth_1.protect, async (req, res) => {
    try {
        const sale = await db_1.default.sale.findUnique({
...
router.post("/returns", auth_1.protect, async (req, res) => {
    const { saleId, items, // [{ saleItemId?, productId, quantity, reason? }]
    refundMethod, // CASH | CARD | MOBILE | CREDIT_ADJUST
    reason, notes } = req.body;
    ...
    try {
        const refundResult = await db_1.default.$transaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                const originalItem = sale.items.find((si) => si.productId === item.productId || (item.saleItemId && si.id === item.saleItemId));
                if (!originalItem) {
                ...
                if (qty > remainingQty) {
                    throw new Error(`Cannot return ${qty} of ${originalItem.product?.name || originalItem.productId}. Only ${remainingQty} remaining.`);
                }
            ...
            if (sale.customerId && (creditPortion > 0 || method === "CREDIT_ADJUST")) {
                const creditReduce = method === "CREDIT_ADJUST" ? refundValue : creditPortion;
                if (creditReduce > 0) {
...
router.post("/:id/emi", auth_1.protect, upload.fields([
    { name: "cnicFront", maxCount: 1 },
    { name: "cnicBack", maxCount: 1 },
    { name: "cheque", maxCount: 1 }
]), async (req, res) => {
    const { id } = req.params;
    const { guarantorName, guarantorPhone, guarantorAddress, months, interestRate, downPayment } = req.body;
    ...
    try {
        ...
        const updatedPaymentStatus = updatedPaidAmount >= totalPrincipal ? "PAID" : updatedPaidAmount > 0 ? "PARTIAL" : "UNPAID";
        ...
        const emiContract = await db_1.default.$transaction(async (tx) => {
            await tx.sale.update({
                where: { id },
                data: {
...
router.post("/:id/installments/:installmentId/pay", auth_1.protect, async (req, res) => {
    const { id, installmentId } = req.params;
    try {
        const installment = await db_1.default.emiInstallment.findUnique({
        ...
        const updatedInstallment = await db_1.default.$transaction(async (tx) => {
